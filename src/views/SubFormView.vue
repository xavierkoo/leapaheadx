        ...
        <div class="row">
            <div id="left-palette">
                <div class="rounded-3 p-5 mt-4" style="background-color: #0f1726">
                    ...
                    <div class="rounded-3 p-3 form-container" style="background-color: #1a263c">
                        <div v-for="(inputComponent, index) in formComponents" :key="index">
                            <div
                                class="form-component my-5"
                                style="
                                    border-radius: 5px;
                                    padding: 10px;
                                    margin-top: 25px;
                            ...
                            >
                                <label class="mb-2 text-dark"
                                    ><strong>{{ inputComponent.type }}</strong></label
                                >
                                <br />
                                <label class="text-dark">Question:</label>
                                <input
                                    v-model="inputComponent.question"
                                    class="form-control"
                                    type="text"
                                ...
                                <div
                                    v-if="
                                        ['Drop-Down Menu', 'Check Box', 'Radio Button'].includes(
                                            inputComponent.type
                                        )
                                    "
                                >
                                    <div class="additional-inputs-container row">
                                        <div
                                            v-for="(
                                                option, optionIndex
                                            ) in inputComponent.options.sort()"
                                            :key="optionIndex"
                                            class="col-6 additional-input my-2"
                                        >
                                            <label class="text-dark"
                                                >Option {{ optionIndex + 1 }}</label
                                            >
                                            <input
                                                v-model="option.split(',')[0]"
                                                class="form-control"
                                                type="text"
                                                placeholder="Option"
                                                disabled
                                ...
                                <div class="form-check mt-2">
                                    <input
                                        v-model="inputComponent.required"
                                        class="form-check-input"
                                        type="checkbox"
                                        name="required"
                                        disabled
                                    />
                                    <label class="text-dark" for="required-checkbox"
                                        >Required</label
                                    >
                                </div>
                            </div>
...
const getSubForm = async () => {
    try {
        const response = await axios.get('http://localhost:8080/api/subformcanvas/' + canvasUuid)
