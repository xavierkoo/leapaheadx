...
<!-- <template>
  ...
  <template>
    <form>
      <div v-for="(input, index) in inputs" :key="index">
        <label :for="input.question">{{ input.question }}</label> // add Heading tag to wrap input.question
        <select v-if="input.type === 'select'" :name="input.question">
          <option v-for="(option, i) in input.options" :key="i" :value="option.value">{{ option.label }}</option>
        </select>
        <div v-else-if="input.type === 'radio'">
          <div v-for="(option, i) in input.options" :key="i">
            <input type="radio" :name="input.question" :value="option.value" :id="`${input.question}-${i}`" v-model="input.value">
